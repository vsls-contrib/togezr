...
export const renderGuestsGithub = async (guests: IGuestWithSessions[]) => {
    const resultPromises = guests.map(async (guest) => {
        ...
        if (!username || isEmail(username)) {
            return `<img src="${DEFAULT_GITHUB_AVATAR}" width="40" alt="${guest.data.emailAddress}" title="${guest.data.displayName} (${guest.data.emailAddress}) ${suffix}"></img>`;
        }

        const avatarUrl = await getGithubAvatar(username);

        const image = `<img src="${avatarUrl}" width="40" alt="${username}" title="${guest.data.displayName} (@${username}) ${suffix}"></img>`;
